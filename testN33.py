...
def remove_third_cluster(image, segmented_image):
    # Copiar la imagen segmentada y eliminar el tercer cluster (índice 2)
    modified_segmented_image = segmented_image.copy()
    modified_segmented_image[modified_segmented_image == 2] = -1  # Establecer el tercer cluster como fondo

    # Crear la superposición con color
    overlay = color.label2rgb(modified_segmented_image, image=image, bg_label=0, alpha=0.3)
